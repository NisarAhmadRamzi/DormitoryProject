...
function CreateBookForm({ bookToEdit = {}, onCloseModal }) {
  ...
  const { mutate, isLoading } = useMutation({
    ...
    onError: (err) => {
      toast.error(err?.response?.data?.message || t('createBookForm.error'))
    },
  })
        ...
        <Select
          ...
          {...register('status', {
            ...
            validate: (value) =>
              [
                'Available',
                'Not Available!!! all books loaned',
                '100Af fine for delay',
              ].includes(value) || t('createBookForm.statusInvalid'),
          })}
        >
          <option value="">-- {t('createBookForm.selectStatus')} --</option>
          <option value="Available">{t('createBookForm.available')}</option>
          <option value="Not Available!!! all books loaned">
            {t('createBookForm.notAvailable')}
          </option>
          {/* <option value="100Af fine for delay">
            {t('createBookForm.fineForDelay')}
          </option> */}
        </Select>
        {errors?.status && <Error>{errors.status.message}</Error>}
      </FormRow>
